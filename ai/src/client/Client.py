...
class Client():
    ...
    def parse_response(self, response: str):
        if "WELCOME" in response:
            ...
            if (self.ai.mentor == True):
                self.ai.check_if_evolution()
                self.ai.mentor = False
            else:
                for _ in range(10):
                    self.ai.prog_action.append("Right")
            ...
            while True:
                ...
                for _, mask in event:
                    if mask & selectors.EVENT_READ:
                        tab_data = self.socket.recv(BUFFER_SIZE).decode(UTF_UNICODE).split("\n")
                        for recieve_data in tab_data:
                            if recieve_data.strip() in self.ai.last_message or recieve_data == "":
                                continue
                            print("\nEVENT READ")
